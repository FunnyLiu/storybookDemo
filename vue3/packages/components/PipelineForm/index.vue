...
<template>
  ...
  >
    <el-tabs
      :modelValue="activeForm"
      :before-leave="handleLeaveTab"
    >
      <div :key="form.key" v-for="form in filtedFormDataList">
        <el-tab-pane
          :name="`form-${form.key}`"
        >
          <template #label>
    ...
    <template #footer>
      ...
      >
        ...
        <el-button
          v-if="!formDisabled && showSubmitBtn"
          type="primary"
          size="small"
          @click="handleSave"
...
import { find, findIndex, last } from '../../utils/array'
/**
 * 流水线表单组件
 *
 * 每个子表单通过单独组件来维护
 *
 * 使用说明参考：https://github.com/EddyPan/vue-complex-form
 *
 */
export default {
  name: 'DoPipelineForm',
  ...
  computed: {
    ...
    showPreviousBtn() {
      ...
      return findIndex(this.filtedFormDataList, ['key', key]) > 0
    },
    showNextBtn() {
      const key = this.activeForm.split('-')[1]
      return findIndex(this.filtedFormDataList, ['key', key]) < this.filtedFormDataList.length - 1
    },
    showSubmitBtn() {
      const key = this.activeForm.split('-')[1]
      return findIndex(this.filtedFormDataList, ['key', key]) === this.filtedFormDataList.length - 1
  ...
  watch: {
    ...
    'filtedFormDataList.length': {
      handler() {
        const key = this.activeForm.split('-')[1]
        if (findIndex(this.filtedFormDataList, ['key', key]) < 0) {
          this.activeForm = `form-${last(this.filtedFormDataList).key}`
        }
      }
    }
  },
  created() {
    this.formDataList = cloneDeep(this.data)
  ...
  methods: {
    ...
    previous() {
      const key = this.activeForm.split('-')[1]
      this.activeForm = `form-${this.filtedFormDataList[findIndex(this.filtedFormDataList, ['key', key]) - 1].key}`
    },
    next() {
      const key = this.activeForm.split('-')[1]
      this.activeForm = `form-${this.filtedFormDataList[findIndex(this.filtedFormDataList, ['key', key]) + 1].key}`
    },
    async handleLeaveTab(activeName, oldActiveName) {
      const fromKey = oldActiveName.split('-')[1]
      const toKey = activeName.split('-')[1]
      const from = findIndex(this.filtedFormDataList, ['key', fromKey])
      const to = findIndex(this.filtedFormDataList, ['key', toKey])
      // 向后进行翻页，前面的表单必须通过检测，否则停在第一个未通过检测的tab页面
      if (to > from) {
        for (let i = from; i < to; i++) {
          const key = this.filtedFormDataList[i].key
          this.$refs[key][0].validForm().then(res => {
            if(!res) {
    ...
    handleValidate(formKey, isValid) {
      ...
      // this.$set(find(this.formDataList, ['key', formKey]), 'valid', isValid)
    },

    // 将发生变换的数据装在到linkChannel中
    handleFieldChange(key, value) {
      this.linkChannel[key] = value
